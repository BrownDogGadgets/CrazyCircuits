...
void loop() {
  ...
  if (buttonValue == LOW) {
    ...
    // Each of the following plays a note and pauses one second. 
    // For more time to run the maze, copy and paste more tones and delay.
    // For LESS time, delete some of the tones and delay.
    // Using the "tone(x,x,x) command, the first number is the pin number the speaker is on, 
    // the second number is frequency (pitch) and the third number is the duration of the tone.
