...
int servoPIN = 3;
int servoTurnTimeForward = 2000; // time servo will run this many milliseconds
int servoTurnTimeReverse = 2000; // time servo will run this many milliseconds
...
void loop() {
  ...
  if ((theDistance > 0) and (theDistance < triggerDistance)) {
    ...
    for (int fadeDValue = 255; fadeDValue > 10; fadeDValue -= 10) {
    ...
    sendCommand(CMD_PLAY_W_VOL, 0X0F01); // play the first file with volume 15 class
    delay(4000); // delay for audio to finish playing
    ...
    for (int fadeUValue = 10; fadeUValue < 255; fadeUValue += 10) {
